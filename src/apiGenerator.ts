...
import { EnvModel, SmartContractControllers, SmartContractModels, SmartApiController, SmartRoutesModels, SmartRouterModels, SmartApiSwaggerYamlModels } from './models';
...
export class ApiGenerator {
    ...
    public async generate() {
      console.log("generating stub..");
       ...
       apiGen.on('close', async (code) =>  {
        ...
        console.log("to run the application: npx lerna run dev --scope " + this.chaincode + "-app --stream");
     });
    }

    private async copyTsConfig() {
      await SysWrapper.copyFile(join(__dirname, '../templates/_tsconfig-api.json.ejs'), join(process.cwd(), `.`) + `/packages/${this.chaincode}-app/tsconfig.json`);
    }

    private async copySelfgenfabriccontext() {
      await SysWrapper.copyFile(join(__dirname, '../templates/_selfgenfabriccontext.ts.ejs'), join(process.cwd(), `.`) + `/packages/${this.chaincode}-app/server/selfgenfabriccontext.ts`);
    }
    ...
    private async generateController() {
      ...
      let smartApiController = new SmartApiController(this.name, this.chaincode, null, methods,controllerNames[0] , false);
    ...
    private async generateSwaggerYaml() {
      ...
      let controllerNames = await ReflectionUtils.getClassNames(controllersPattern);
      let methods = await ReflectionUtils.getClassMethods(controllersPattern, controllerNames[0]);
      let smartApiSwaggerModels = new SmartApiSwaggerYamlModels(this.name, this.chaincode, this.projectName, modelNames, modelClasses, methods, false);
